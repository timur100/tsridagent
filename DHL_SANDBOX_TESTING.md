...
1. **Öffnen Sie in Ihrem Browser**:
   ```
   https://asset-kit-builder.preview.emergentagent.com/portal/admin
   ```
...
#### 1. Health Check
```bash
curl -s "https://asset-kit-builder.preview.emergentagent.com/api/dhl/health" | jq
```
...
```bash
curl -s "https://asset-kit-builder.preview.emergentagent.com/api/dhl/shipments/mock" | jq
```

#### 3. Test-Sendung erstellen (Sandbox)
```bash
curl -X POST "https://asset-kit-builder.preview.emergentagent.com/api/dhl/shipments" \

...
#### 1. Health Check
```bash
curl -s "https://tenant-security-4.preview.emergentagent.com/api/dhl/health" | jq
```
...
```bash
curl -s "https://tenant-security-4.preview.emergentagent.com/api/dhl/shipments/mock" | jq
```

#### 3. Test-Sendung erstellen (Sandbox)
```bash
curl -X POST "https://tenant-security-4.preview.emergentagent.com/api/dhl/shipments" \

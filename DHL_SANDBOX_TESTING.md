...
1. **Öffnen Sie in Ihrem Browser**:
   ```
   https://devops-central-17.preview.emergentagent.com/portal/admin
   ```
...
#### 1. Health Check
```bash
curl -s "https://devops-central-17.preview.emergentagent.com/api/dhl/health" | jq
```
...
```bash
curl -s "https://devops-central-17.preview.emergentagent.com/api/dhl/shipments/mock" | jq
```

#### 3. Test-Sendung erstellen (Sandbox)
```bash
curl -X POST "https://devops-central-17.preview.emergentagent.com/api/dhl/shipments" \

...
#### 1. Health Check
```bash
curl -s "https://tablet-fleet-hub.preview.emergentagent.com/api/dhl/health" | jq
```
...
```bash
curl -s "https://tablet-fleet-hub.preview.emergentagent.com/api/dhl/shipments/mock" | jq
```

#### 3. Test-Sendung erstellen (Sandbox)
```bash
curl -X POST "https://tablet-fleet-hub.preview.emergentagent.com/api/dhl/shipments" \

...
```bash
curl -s "https://offline-agent.preview.emergentagent.com/api/dhl/shipments/mock" | jq
```

#### 3. Test-Sendung erstellen (Sandbox)
```bash
curl -X POST "https://offline-agent.preview.emergentagent.com/api/dhl/shipments" \

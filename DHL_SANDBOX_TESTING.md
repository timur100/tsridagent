...
#### 1. Health Check
```bash
curl -s "https://tenant-filter-search.preview.emergentagent.com/api/dhl/health" | jq
```
...
```bash
curl -s "https://tenant-filter-search.preview.emergentagent.com/api/dhl/shipments/mock" | jq
```

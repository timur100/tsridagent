...
1. **Öffnen Sie in Ihrem Browser**:
   ```
   https://electron-regula-hub.preview.emergentagent.com/portal/admin
   ```
...
#### 1. Health Check
```bash
curl -s "https://electron-regula-hub.preview.emergentagent.com/api/dhl/health" | jq
```
...
```bash
curl -s "https://electron-regula-hub.preview.emergentagent.com/api/dhl/shipments/mock" | jq
```

#### 3. Test-Sendung erstellen (Sandbox)
```bash
curl -X POST "https://electron-regula-hub.preview.emergentagent.com/api/dhl/shipments" \

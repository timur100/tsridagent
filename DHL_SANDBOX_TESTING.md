...
#### 1. Health Check
```bash
curl -s "https://kit-workflow-app.preview.emergentagent.com/api/dhl/health" | jq
```
...
```bash
curl -s "https://kit-workflow-app.preview.emergentagent.com/api/dhl/shipments/mock" | jq
```

#### 3. Test-Sendung erstellen (Sandbox)
```bash
curl -X POST "https://kit-workflow-app.preview.emergentagent.com/api/dhl/shipments" \

...
from models.sla import SLAStatus, DEFAULT_SLA_CONFIGS
from utils.db import tickets_collection
from utils.auth import verify_token

router = APIRouter(prefix="/sla", tags=["SLA"])

def calculate_sla_status(ticket: dict) -> SLAStatus:
    """Calculate SLA status for a ticket"""
    priority = ticket.get("priority", "medium")
    sla_config = DEFAULT_SLA_CONFIGS.get(priority)
    
    if not sla_config:
        sla_config = DEFAULT_SLA_CONFIGS["medium"]
    ...
    # Get first response time (from comments)
    first_response_at = None
    comments = ticket.get("comments", [])
    if comments:
        # Find first comment by admin/staff
    ...
    return SLAStatus(
        ...
        response_time_remaining=response_time_remaining,
        resolution_time_remaining=resolution_time_remaining
    )

        ...
        <DialogContent className={`max-w-2xl ${isDark ? 'bg-[#2d2d2d] text-white border-gray-700' : 'bg-white'}`}>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <Select 
                value={bulkPrintSelectedTemplateId} 
                onValueChange={handleBulkPrintTemplateChange}
            ...
            {/* Vorschau des ersten Labels */}
            {bulkPrintAssets.length > 0 && (
              <div>
                <p className={`text-sm mb-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                  Vorschau (erstes Label):
                </p>
                <div className="flex justify-center">
                  <LabelPreview 
                    asset={bulkPrintAssets[0]} 
                    template={bulkPrintSelectedTemplate} 
                    isDark={isDark} 
                  />
                </div>
              </div>
            ...
            <div>
              <p className={`text-sm mb-2 ${isDark ? 'text-gray-400' : 'text-gray-600'}`}>
                Ausgewählte Geräte ({bulkPrintAssets.length}):
              </p>
              <div className={`max-h-40 overflow-y-auto rounded-lg border ${isDark ? 'bg-[#1a1a1a] border-gray-700' : 'bg-gray-50 border-gray-200'}`}>
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {bulkPrintAssets.map((asset, idx) => (
                      <tr 
                        key={asset.manufacturer_sn}
                        className={`border-t ${isDark ? 'border-gray-700' : 'border-gray-200'} ${bulkPrinting && idx === bulkPrintIndex ? 'bg-blue-500/20' : ''} ${bulkPrinting && idx < bulkPrintIndex ? 'opacity-50' : ''}`}
                      >
                        <td className="px-3 py-2">
                          {bulkPrinting && idx === bulkPrintIndex ? (
          ...
          <DialogFooter className="gap-2">
            ...
            <Button 
              ...
              disabled={bulkPrinting || bulkPrintLoading}
            >
              {bulkPrinting ? (
                <>
                  <Loader2 className="h-4 w-4 mr-2 animate-spin" />
                  Drucke {bulkPrintIndex + 1}/{bulkPrintAssets.length}...
                </>
              ) : (
                <>
                  <Printer className="h-4 w-4 mr-2" />

...
const TenantInPreparationPage = () => {
  ...
  const loadInPreparationItems = async () => {
    ...
    try {
      ...
      // Response structure: {success: true, data: {success: true, data: {summary: {...}, devices: [...]}}}
      let allDevices = [];
      if (Array.isArray(devicesResponse)) {
        allDevices = devicesResponse;
      } else if (devicesResponse?.data?.data?.devices && Array.isArray(devicesResponse.data.data.devices)) {
        // Double nested data structure
        allDevices = devicesResponse.data.data.devices;
      } else if (devicesResponse?.data?.devices && Array.isArray(devicesResponse.data.devices)) {
        // Single nested data structure

...
const StockPortalBarcode = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch(`${process.env.REACT_APP_BACKEND_URL || 'https://agent-control-desk-2.preview.emergentagent.com'}/api/fulfillment/auth/login?email=${loginForm.email}&password=${loginForm.password}`, {
        method: 'POST',
      });
  ...
  const fetchOrders = async () => {
    try {
      const backendUrl = process.env.REACT_APP_BACKEND_URL || 'https://agent-control-desk-2.preview.emergentagent.com';

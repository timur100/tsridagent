...
const InPreparationOverviewPage = () => {
  ...
  const filterItems = () => {
    ...
    if (searchTerm) {
      ...
      filteredDevs = filteredDevs.filter(device =>
        ...
        device.tenant_name?.toLowerCase().includes(term) ||
    ...
    if (searchTerm) {
      ...
      filteredLocs = filteredLocs.filter(location =>
        ...
        location.tenant_name?.toLowerCase().includes(term) ||
  ...
  const handleDeviceClick = (device) => {
    navigate(`/portal/admin/tenants/${device.tenant_id}/devices/${device.device_id}`);
  };

  const handleLocationClick = (location) => {
    navigate(`/portal/admin/tenants/${location.tenant_id}/locations/${location.location_code}`);
  };
    ...
    <div className={`min-h-screen p-8 ${theme === 'dark' ? 'bg-[#1a1a1a]' : 'bg-gray-50'}`}>
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className={`text-3xl font-bold ${theme === 'dark' ? 'text-white' : 'text-gray-900'}`}>
              In Vorbereitung - Übersicht
            </h1>
            <p className={`text-sm mt-1 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-600'}`}>
              Alle Geräte und Standorte in Vorbereitung über alle Mandanten
            </p>
          </div>
        </div>
      </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-purple-500">
                {new Set([...devices.map(d => d.tenant_id), ...locations.map(l => l.tenant_id)]).size}
              </p>
            </div>
      ...
      <Card className={`p-4 rounded-xl mb-6 ${
        ...
        <div className="relative">
          ...
          <input
            type="text"
            placeholder="Suchen nach Geräte-ID, Mandant, Standort, Stadt..."
            value={searchTerm}
            ...
            <table className="w-full">
              <thead className={theme === 'dark' ? 'bg-[#1a1a1a]' : 'bg-gray-50'}>
                <tr>
                  ...
                  >
                    Mandant <SortIcon columnKey="tenant_name" />
                  </th>
                  <th 
      ...
      {!loading && totalItems === 0 && (
        <Card className={`p-12 text-center rounded-xl ${
          ...
          <p className={`${theme === 'dark' ? 'text-gray-500' : 'text-gray-500'}`}>
            Aktuell befinden sich keine Geräte oder Standorte im Status "In Vorbereitung"
          </p>
        </Card>
      )}
...
export default InPreparationOverviewPage;
